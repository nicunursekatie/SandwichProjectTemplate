...
router.post("/kudos", async (req, res) => {
  try {
    ...
    const { recipientId, contextType, contextId, entityName, content } =
      result.data;
...
router.get("/unread", async (req, res) => {
  try {
    ...
    const incompleteMessages = messages.filter(
      (msg) => !msg || !msg.senderName || !msg.content,
    );
    if (incompleteMessages.length > 0) {
      console.error(
        "Found incomplete messages being sent to frontend:",
        incompleteMessages,
      );
    }

    console.log(
      "Sending messages to frontend. Total:",
      messages.length,
      "Valid:",
      messages.filter((msg) => msg && msg.senderName && msg.content).length,
    );
...
router.get("/messages", async (req, res) => {
  try {
    ...
    if (contextType && contextType !== "all") {
      ...
      const contextMessages = await messagingService.getContextMessages(
        "direct",
        user.id,
      );
